...
st.write("시간/조건별 R·S 이성질체 정량 비교, 이상 탐지(Anomaly Detection) 및 Methods-to-Conclusion 논문형 보고서를 생성합니다.")
...
if uploaded_files:
    ...
    data_tab1, data_tab2, data_tab3 = st.tabs(["📈 오버레이 스펙트럼", "📊 시간대별 정량 비교(대칭성 및 이동량)", "📋 전체 핵심 Peak 요약"])
    ...
    for g_key, forms in time_groups.items():
        ...
        if r_peaks and s_peaks:
            ...
            shift_penalty = max(0, 100 - (peak_shift * 10)) # 파장 차이가 크면 감점
    ...
    # ==========================================
    # 3. OpenAI API 연동: 완벽한 논문형 리포트 자동 생성
    # ==========================================
    st.write("---")
    st.header("🤖 AI 수석 연구원: 논문형 심층 분석 및 후속 제안 리포트")
    st.write("Methods, Results, Discussion, Conclusion 형식의 완벽한 논문 구조 리포트 및 Word 호환 보고서를 생성합니다.")
    
    if st.button("🚀 AI 논문형 리포트 및 다운로드 파일 생성"):
        if not api_key:
            st.error("⚠️ 좌측 사이드바에 OpenAI API Key를 입력해 주세요!")
        elif not peak_summary:
            st.warning("⚠️ 분석할 피크 데이터가 없습니다.")
        else:
            with st.spinner("AI가 실험의 의미, 이상 탐지, 다음 실험 제안을 포함한 논문형 리포트를 작성 중입니다..."):
                try:
                    ...
                    system_prompt = (
                        ...
                        "- 다음 실험 제안을 위해 **구체적인 시간, 온도, 농도 조건**을 체크박스(- [ ]) 형태로 제시 (예: 110분 추가 측정, 온도 5도 상향 조정, 샘플 농도 최적화 등)."
                    )
                    ...
                    # Word/PDF 호환용 Markdown 파일 생성
                    st.write("---")
                    st.subheader("📥 보고서 파일 다운로드")
                    st.info("💡 다운로드하신 `.md` (마크다운) 파일은 Microsoft Word에서 바로 열거나 깔끔하게 PDF로 변환할 수 있는 범용 문서 포맷입니다.")
                    
                    md_file = io.BytesIO(report_content.encode('utf-8'))
                    st.download_button(

...
st.write("조건(Open/Half/Close) 및 어닐링 시간대별 R/S 이성질체 대칭성 정량화 및 구조화된 연구 보고서 자동 생성 시스템")
...
if uploaded_files:
    ...
    data_tab1, data_tab2, data_tab3 = st.tabs(["📈 오버레이 스펙트럼", "📊 조건/시간별 대칭성 지표(%)", "📋 핵심 Peak 요약"])
    ...
    for f in uploaded_files:
        try:
            ...
            # 만약 조건이나 시간이 비어있으면 파일명 전체 조합으로 그룹 키 생성
            ...
            sample_peaks = []
            for p in peaks_pos:
                sample_peaks.append({"샘플명": f.name, "형태": form, "조건": condition, "시간": time_val, "유형": "Positive", "파장(nm)": round(x[p], 2), "강도(Y)": round(y[p], 3), "AbsY": abs(y[p])})
            for p in peaks_neg:
                sample_peaks.append({"샘플명": f.name, "형태": form, "조건": condition, "시간": time_val, "유형": "Negative", "파장(nm)": round(x[p], 2), "강도(Y)": round(y[p], 3), "AbsY": abs(y[p])})
            
            # 그룹별 매칭을 위한 딕셔너리 안전 적재
    ...
    # 방어 코드가 적용된 대칭성 계산 로직 (KeyError 원천 차단)
    symmetry_results = []
    for g_key, forms in time_groups.items():
        ...
        if r_peaks and s_peaks:
            ...
            try:
                r_avg_intensity = np.mean([p["AbsY"] for p in r_peaks])
                s_avg_intensity = np.mean([p["AbsY"] for p in s_peaks])
                intensity_ratio = min(r_avg_intensity, s_avg_intensity) / max(r_avg_intensity, s_avg_intensity) * 100
            except:
                intensity_ratio = 70.0
            
            symmetry_score = round((count_match * 0.4) + (intensity_ratio * 0.6), 1)
            symmetry_score = max(10.0, min(98.5, symmetry_score))
    ...
    with data_tab2:
        st.subheader("📊 조건/시간대별 R/S 이성질체 거울상 대칭성 지표")
        if symmetry_results:
            sym_df = pd.DataFrame(symmetry_results)
            st.dataframe(sym_df, use_container_width=True)
            st.info("💡 **대칭성 지표 안내:** R-form과 S-form 간의 피크 위치, 개수, 그리고 강도 비율을 수학적으로 비교하여 산출된 백분율 점수입니다.")
        else:
            st.warning("대칭성을 비교할 수 있는 데이터가 부족합니다.")

    with data_tab3:
        if peak_summary:
            peak_df = pd.DataFrame(peak_summary)
            # AbsY 컬럼이 있다면 제거하고 표에 표시
            if "AbsY" in peak_df.columns:
                peak_df = peak_df.drop(columns=["AbsY"])
    ...
    st.write("산출된 대칭성 지표(%)와 피크 데이터를 바탕으로, 한눈에 들어오는 가독성 높은 연구 리포트를 생성합니다.")
            ...
            with st.spinner("AI가 가독성이 뛰어난 전문 연구 보고서를 작성하고 있습니다..."):
                try:
                    ...
                    system_prompt = (
                        ...
                        "사용자는 다양한 조건(Open/Half/Close)과 어닐링 시간별로 R-form과 S-form의 CD 스펙트럼 변화를 분석하고 있습니다. "
                        "결과 리포트는 가독성을 극대화하기 위해 이모지(📌, 📊, 🔍, ✅ 등)를 적극 활용하고, 불릿 포인트와 마크다운 박스를 사용하여 아래 구조로 정확히 작성해 주세요:\n\n"
                        "### 📌 Executive Summary\n"
                        "- 실험 결과를 종합하여, 신호 안정성, R/S형 각각의 peak intensity 트렌드, 그리고 대칭성 지표(%)를 인용하여 핵심 내용을 3줄 이내로 요약하세요.\n\n"
                        "### 📊 Overall Recommendation & Technical Insights\n"
                        "- **최적 조건/시간 선정:** 데이터에 기반하여 가장 적합한 조건을 콕 집어 추천하고 그 이유를 설명하세요.\n"
                        ...
                        "- **분자 구조적 해석:** peak가 유지되는 현상이 `Molecular packing`, `Increased crystallinity`, `Reduced conformational disorder` 중 무엇과 연관되는지 설명하세요.\n\n"
                    ...
                    user_prompt = f"### 실험 파일 정보\n{meta_info}\n\n### 대칭성 산출 결과\n{sym_data}\n\n### 핵심 피크 데이터\n{prompt_data}\n\n위 데이터를 바탕으로 지정된 가독성 높은 마크다운 포맷으로 최고급 연구 보고서를 작성해 줘."

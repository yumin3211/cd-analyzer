...
if uploaded_files:
    ...
    # ==========================================
    # 3. OpenAI API 연동: 거울상 대칭성 강제 분석 프롬프트
    # ==========================================
    st.write("---")
    st.header("🤖 LLM 기반 카이랄성 및 증발 제어 공정 심층 해석 리포트")
    st.write("R/S 이성질체 간의 거울상 대칭성(Cotton Effect 일치도)과 비커 개폐 조건에 따른 물리화학적 메커니즘을 정밀 분석합니다.")
    ...
    if st.button("🚀 AI 카이랄 공정 분석 실행"):
        if not api_key:
            ...
            with st.spinner("AI가 R/S형 간의 거울상 대칭성과 Cotton Effect 대칭 오차율을 정밀 계산하고 있습니다..."):
                try:
                    ...
                    system_prompt = (
                        "당신은 카이랄 분광학 및 물리화학 분야의 수석 연구원입니다. "
                        "사용자는 비커 조건(Open/Half/Close) 또는 어닐링 시간에 따른 R-form과 S-form 물질의 카이랄성(거울상 대칭성) 변화를 연구하고 있습니다. "
                    ...
                    user_prompt = f"### 실험 파일 정보\n{meta_info}\n\n### 추출된 피크 데이터\n{prompt_data}\n\n위 데이터를 바탕으로 R/S형 간의 거울상 대칭성(Cotton Effect 대칭 정도)을 최우선으로 하여 심층 분석해 줘."
                    ...
                    st.success("✅ AI 카이랄 대칭성 분석 완료!")
                    st.markdown(f"### 📑 카이랄 분광 대칭성 및 공정 분석 리포트")
